...
- [x] **Confermare la chiamata/risposta univoca per mobile e web.**
      _Nello specifico_: le chiamate API alla piattaforma Next devono essere sicure. Gli unici che possono usarle sono i possessori di una specifica password che va impostata. Ergo - bisogna assicurarsi che sia impossibile fare chiamate agli endpoint esposti di Next, a meno che non si abbia la API key che sfrutteremo da mobile. Così nessun malintenzionato può sballarci niente.
